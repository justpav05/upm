...
use super::{CommonError, SearchInPackageFilesError};
...
fn regex_error_maps_to_invalid_search_pattern() {
    ...
    assert!(matches!(
        ...
        SearchInPackageFilesError::InvalidSearchPattern(_)
    ));
}
...
fn every_own_variant_maps_to_the_documented_error_kind() {
    let cases = [
        (
            SearchInPackageFilesError::Common(CommonError::OutOfMemory),
            ErrorKind::OutOfMemory,
        ),
        (
            SearchInPackageFilesError::InvalidSearchPattern("(".to_owned()),
