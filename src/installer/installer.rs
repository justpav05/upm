...
impl PackageInstaller {
    ...
    fn copy_with_permissions(&mut self, source_path: &Path, dest_path: &Path) -> InstallerResult<()> {
        ...
        ).map_err(|err| InstallerError::IoError(err.to_string().into()))?;
        Ok(())
    }

    fn install_inner(&mut self, package: ExtractedPackage) -> InstallerResult<()> {
        ...
        if !root_path.exists() {
            ...
            return Err(InstallerError::PackageNotFound(self.root_path.clone().into()));
        }

        if !temp_dir_path.exists() {
            self.set_state(InstallerState::Failed);
            return Err(InstallerError::PackageNotFound(self.temp_path.clone().into()));
        }

        for file_path in package.file_list.iter().map(|file_string| PathBuf::from(file_string.as_str())) {
            if !file_path.exists() {
                self.set_state(InstallerState::Failed);
    ...
    fn remove_inner(&mut self, package: &str) -> InstallerResult<()> {
        self.set_state(InstallerState::Preparing);
...
impl Installer for PackageInstaller {
    extern "C" fn install(&mut self, package: ExtractedPackage) -> InstallerStabbyResult<()> {
        self.install_inner(package).into()
    }

    extern "C" fn remove(&mut self, package: StabStr) -> InstallerStabbyResult<()> {
        self.remove_inner(package.as_str()).into()
    }
}
